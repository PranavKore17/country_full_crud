...
// dd($request->all());
   ...
   public function fetchstatepost(Request $request){
   ...
   public function fetchstateget(Request $request){
// print_r('ubfuf');
// $country_id=$cid;
    $cid=$request->post('country_id');
    ...
    $html='<option value="" style="width: 160px;">Select state </option> ';

    foreach($state as $list){
        $html.='<option value="'.$list->id.'" style="width: 160px;"> '.$list->state_name.' </option>';
    }
        echo $html;
   }
