    ...
    <div class="container">
      ...
      @endphp
        ...
        <nav class="navbar navbar-expand-lg navbar-light bg-light" >
            <a class="navbar-brand" href="#">City CRUD</a>
          ...
          <form action="{{route('city.store')}}" method="POST" id='form' >
            ...
            <div class="form-group" data-aos="fade-right">
              <label for="formGroupExampleInput">City name</label><span class="text-danger">*</span>
              @if ($errors->has('name'))
                <span class="text-danger">{{$errors->first('name')}}</span>
              @endif
              <input type="text" class="form-control" name="name" id="name" placeholder="City name" value="{{$data->city_name??old('name')}}">
            </div>
            ...
            {{-- <label id="curr-error" class="error" for="curr"></label> --}}
              <select name="country[]" class="cou" >
                <option value="{{$data->country_id??old('name')}}">@if(isset($data->country_id)){{$data->country_id}}@else Select Country @endif
                  </option>
                @foreach ($country as $test)
                    
                  <option value="{{$test->id}}"> {{$test->country_name}} </option>
                @endforeach
            ...
            @endif 
            {{-- <label id="curr-error" class="error" for="curr"></label> --}}
              <select name="state[]" class="curr">
                <option value="{{$data->state_id??old('name')}}">@if(isset($data->state_id)){{$data->state_id}}@else Select State @endif
                  </option>
            ...
            <a href="{{route('city.index')}}" class="btn btn-danger" data-aos="fade-left">Cancle</a>
          </form>
    ...
    <script>
      $(document).ready(function(){ 
         $(".alert").fadeTo(1000, 500).slideUp(500, function(){
             $(".alert").slideUp(600);
             setTimeout(window.location.href = "{{route('city.index')}}", 1000);
           });
